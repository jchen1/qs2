...
// TODO we should support modifying builds
function ruleExistsInBuild(build: string, ruleName: string, libString: string) {
  if (build.includes(libString)) return true;

  assert(!build.includes(`name = "${ruleName}"`), `auto_deps rule ${ruleName} conflicts with existing rule!`);
  return false;
}
...
function generateBuildFile(build: string, libraries: { ruleName: string, libString: string }[]): string {
  ...
  const buildWithLibs = libraries.reduce((build, { ruleName, libString }) => {
    if (!ruleExistsInBuild(build, ruleName, libString)) {
      build = `${build}
